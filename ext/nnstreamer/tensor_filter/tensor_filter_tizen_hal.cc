...
const char *tizen_hal_subplugin::fw_name = "hal";
...
{
  int ret = hal_ml_create ("tizen-hal", &hal_handle);
  if (ret == HAL_ML_ERROR_INVALID_PARAMETER) {
    throw std::invalid_argument ("Tizen HAL is not supported");
  }

  if (ret != HAL_ML_ERROR_NONE) {
    throw std::runtime_error ("Failed to initialize Tizen HAL ML");
  }
}
...
tizen_hal_subplugin::configure_instance (const GstTensorFilterProperties *prop)
{
   ...
   * parse the backend_name into char* backend_name
   */
  char *backend_name = NULL;
  char *custom_properties_copy = g_strdup (prop->custom_properties);
  char **options = g_strsplit (custom_properties_copy, ",", -1);

  for (guint op = 0; op < g_strv_length (options); ++op) {
    char **option = g_strsplit (options[op], ":", -1);
  ...
  g_free (backend_name); //free backend name after using.
  if (ret == HAL_ML_ERROR_INVALID_PARAMETER) {
    throw std::invalid_argument ("Tizen HAL is not supported");
  }
