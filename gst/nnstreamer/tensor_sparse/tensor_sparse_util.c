 ...
 * @param[out] out pointer of output dense tensor data
...
gst_tensor_sparse_to_dense (GstTensorMetaInfo * meta, gpointer in)
{
  ...
  output = (guint8 *) g_malloc0 (element_size * element_count);
  ...
  // return TRUE;
  return gst_memory_new_wrapped (0, output, element_size * element_count, 0, element_size * element_count, output, g_free);
}
 ...
 * @param[out] out pointer of output sparse tensor data
...
gst_tensor_sparse_from_dense (GstTensorMetaInfo * meta, gpointer in)
{
  ...
  data_type = meta->type;
  nns_logw ("element_size: %d", (int) element_size);
  nns_logw ("element_count: %d", (int) element_count);
  nns_logw ("meta->type: %d", data_type);
  ...
  /** write to out */
  output = g_malloc0 (element_size * nnz + sizeof (guint) * nnz);
  memcpy (output, values, element_size * nnz);
  memcpy ((guint8 *) output + (element_size * nnz), indices, sizeof (guint) * nnz);

  g_free (values);
  g_free (indices);

  return gst_tensor_meta_info_append_header (
            meta,
            gst_memory_new_wrapped (0, output, element_size * nnz + sizeof (guint) * nnz, 0, element_size * nnz + sizeof (guint) * nnz, output, g_free)
  );
}
