    ...
    const GstTensorsInfo * info2)
{
  ...
  for (i = 0; i < NNS_TENSOR_SIZE_LIMIT + NNS_TENSOR_SIZE_EXTRA_LIMIT; i++) {
    ...
    if (info1->num_tensors > i) {
      tmp =
          gst_tensor_get_dimension_string (gst_tensors_info_get_nth_info (
              (GstTensorsInfo *) info1, i)->dimension);
      left =
          g_strdup_printf ("%s [%s]",
          gst_tensor_get_type_string (gst_tensors_info_get_nth_info (
                  (GstTensorsInfo *) info1, i)->type), tmp);
      g_free (tmp);
    } else {
      left = g_strdup ("None");
    }
    ...
    if (info2->num_tensors > i) {
      tmp =
          gst_tensor_get_dimension_string (gst_tensors_info_get_nth_info (
              (GstTensorsInfo *) info2, i)->dimension);
      right =
          g_strdup_printf ("%s [%s]",
          gst_tensor_get_type_string (gst_tensors_info_get_nth_info (
                  (GstTensorsInfo *) info2, i)->type), tmp);
      g_free (tmp);
    } else {
      right = g_strdup ("None");
    }
