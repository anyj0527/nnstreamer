...
TEST_F (NNSFilterSingleTestManyInOut, setInvalidInfo_n)
{
  ...
  for (guint i = 0; i < in_info.num_tensors; i++) {
    gst_tensors_info_get_nth_info (&in_info, i)->type = _NNS_FLOAT32;
    gst_tensor_parse_dimension ("1", gst_tensors_info_get_nth_info (&in_info, i)->dimension);
  }
  ...
  for (guint i = 0; i < out_info.num_tensors; i++) {
    EXPECT_EQ (gst_tensors_info_get_nth_info (&out_info, i)->type, _NNS_FLOAT32);
    EXPECT_EQ (gst_tensors_info_get_nth_info (&out_info, i)->dimension[0], 1U);
    EXPECT_EQ (gst_tensor_info_get_rank (gst_tensors_info_get_nth_info (&out_info, i)), 1U);
  }
